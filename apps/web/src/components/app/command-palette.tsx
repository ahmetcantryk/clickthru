...
export function CommandPalette({
  ...
  const actions = useMemo<Action[]>(() => {
    ...
    const nav: Action[] = [
      { id: 'new', label: 'Yeni demo oluştur', icon: <Plus className="h-4 w-4" />, group: 'Eylemler', run: () => { onClose(); onNew(); } },
      { id: 'home', label: 'Ana sayfa', icon: <Home className="h-4 w-4" />, group: 'Git', run: go('/workspaces') },
      { id: 'lib', label: 'Kitaplık', icon: <BookOpen className="h-4 w-4" />, group: 'Git', run: go('/library') },
      { id: 'ins', label: 'İçgörüler', icon: <Sparkles className="h-4 w-4" />, group: 'Git', run: go('/insights') },
      { id: 'set', label: 'Ayarlar', icon: <Settings className="h-4 w-4" />, group: 'Git', run: go('/settings') },
    ];
    return [...demoActions, ...nav];
  }, [demos, router, onClose, onNew]);
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-start justify-center p-4 pt-[12vh]">
      <button type="button" aria-label="Kapat" onClick={onClose} className="absolute inset-0 cursor-default bg-canvas/70 backdrop-blur-sm" />
      <div className="relative w-full max-w-xl overflow-hidden rounded-2xl border border-hairline bg-surface-elevated shadow-soft animate-[fadein_.12s_ease]">
        <div className="flex items-center gap-2.5 border-b border-hairline px-4">
          ...
          <input
            ...
            placeholder="Demo ara veya bir yere git…"
        ...
        <div className="max-h-80 overflow-auto p-1.5">
          {filtered.length === 0 ? (
            <div className="px-3 py-10 text-center text-sm text-ink-faint">Sonuç yok</div>
          ) : (
            filtered.map((a, i) => {
              ...
              return (
                <div key={a.id}>
                  ...
                  <button
                    ...
                    onClick={a.run}
                    className={cn('flex w-full items-center gap-3 rounded-lg px-2.5 py-2 text-left transition-colors', i === hi ? 'bg-accent-muted text-ink' : 'text-ink-muted')}
                  >

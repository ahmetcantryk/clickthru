    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
      ...
      <div className="relative w-full max-w-3xl">
        <button
          ...
          onClick={onClose}
          aria-label="Kapat"
          className="absolute -top-2 right-0 flex h-9 w-9 items-center justify-center rounded-lg text-ink-muted hover:bg-surface-subtle hover:text-ink"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        {view === 'choose' ? (
          <div className="animate-[fadein_.25s_ease]">
            <h2 className="text-center text-3xl font-extrabold tracking-tight text-ink">Nasıl başlamak istersin?</h2>
            <div className="mt-10 grid gap-5 sm:grid-cols-2">
              <ChoiceCard
                icon={<Upload className="h-7 w-7" strokeWidth={1.5} />}
                title="Sıfırdan başla"
                desc="Bilgisayarından görsel yükle, adım adım kur."
                onClick={() => router.push('/studio?new=1')}
              />
              <ChoiceCard
                icon={<Chrome className="h-7 w-7" strokeWidth={1.5} />}
                title="Chrome Uzantısı"
                desc="Tarayıcında herhangi bir siteyi kaydet."
                badge="Popüler"
          ...
          <div className="animate-[fadein_.25s_ease] overflow-hidden rounded-3xl border border-hairline bg-surface shadow-soft">
            <div className="grid md:grid-cols-2">
              <div className="p-9">
                ...
                <h2 className="text-2xl font-extrabold tracking-tight text-ink">clickthru uzantısını yükle</h2>
                <ul className="mt-6 space-y-3">
                  {['Kaydı tek tıkla başlat', 'Her tıklama otomatik bir adım olur', 'Edge, Brave ve Opera ile de çalışır'].map((t) => (
                ...
                <div className="mt-8 flex flex-col items-start gap-3">
                  ...
                  >
                    <Chrome className="h-[18px] w-[18px]" /> Uzantıyı yükle
                  </a>
                  <button
                    type="button"
                    onClick={() => router.push('/studio?new=1')}
              ...
              <div className="relative hidden items-center justify-center overflow-hidden bg-gradient-to-b from-accent-muted to-transparent p-8 md:flex">
                ...
                <div className="absolute right-12 top-16 flex items-center gap-2 rounded-xl bg-ink/85 px-3 py-2 text-xs font-semibold text-white shadow-soft">
                  <span className="h-2 w-2 rounded-full bg-accent" /> Ekranını yakala
                </div>
              </div>
            </div>
          </div>
...
function ChoiceCard({ icon, title, desc, onClick, badge }: { icon: React.ReactNode; title: string; desc: string; onClick: () => void; badge?: string }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'group relative flex flex-col items-center gap-4 rounded-3xl border bg-surface p-9 text-center transition-all hover:-translate-y-1 hover:shadow-soft',
        badge ? 'border-accent-ring' : 'border-hairline hover:border-hairline-strong',
      )}
    >
      {badge && (
      ...
      <span className="flex h-16 w-16 items-center justify-center rounded-2xl bg-surface-subtle text-ink transition-colors group-hover:bg-accent-muted group-hover:text-accent">
        {icon}
      </span>
      <div>

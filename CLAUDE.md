...
- `2026-06-02` — **Faz 2 başladı: Export** (kullanıcı): **HTML embed tam çalışır** — `/embed/{id}` chrome'suz tam-ekran player (`?export=1` → kontroller gizli + otomatik oynatma, render kaydı için). Studio "Paylaş" → **`ExportDialog`** (Bağlantı / Embed / Video&GIF): iframe snippet + canlı önizleme. **Video/GIF sunucu-taraflı:** `services/render-worker` (Playwright headless Chromium ile `/embed/{id}?export=1` kaydı → ffmpeg ile mp4/gif) — **bilinçli olarak pnpm workspace DIŞINDA** (`pnpm-workspace.yaml`'de `!services/render-worker`; ağır Playwright deps Vercel/kök kurulumunu bozmasın, standalone deploy). Web `/api/export` route'u `RENDER_WORKER_URL`'e proxy yapar; env yoksa **503 → "yapılandırılmadı"** (link + embed yine çalışır). `Player`'a opsiyonel `hideControls` + `autoAdvanceMs` (default off → studio/play/preview değişmedi). `share-dialog.tsx` → `export-dialog.tsx`. Kurulum: `services/render-worker/README.md`.
...
- `2026-06-02` — **Studio + Player de TR/EN** (kullanıcı, önceki "studio Türkçe kalır" kararını **override eder**): Editör + player tamamen iki dilli, **default İngilizce**. i18n'e `studio` namespace'i eklendi (en kanonik + tr ayna): üst bar (Doğrula/Önizle/Paylaş/durum), panel toggle'ları, adım paneli (kart başlıkları `Adım N`, 3-nokta menü, tür çipi, boş durum, "PC'den görsel ekle"), canvas dock (Hotspot/Callout/Metin/Focus/Sil + "ekle/zaten ekli" title'ları), inspector (tüm section/field/seçenek: Design/Demo/Çerçeve/Arka plan/wrapper'lar, Hotspot/Callout/Focus/Metin editörleri, ease Yumuşak/Hızlı/Yavaş, pointer Üst/Alt/Sol/Sağ/Yok, "Tümüne uygula"), boş tuval mesajı, player (boş durum, Kapat, ileri/geri/adım aria, callout İleri/Geri via shared `Callout` `nextLabel/backLabel` prop'ları), bozuk medya fallback'i. Studio editör içeriği (örnek demo metinleri) veri olduğundan çevrilmedi. `BrokenMedia` artık `'use client'`. Build yeşil (13 route), tsc temiz, 31 test geçer.
- `2026-06-02` — **App TR/EN i18n, varsayılan İngilizce** (kullanıcı): Tüm **app + auth** ekranları iki dilli oldu, **default her zaman İngilizce**. Yeni `apps/web/src/lib/i18n.tsx` — **`en` kanonik tip kaynağı** (`type Copy = typeof en`), `tr: Copy` aynalar (TS parity'yi zorlar); fonksiyonlu girişler (interpolasyon/çoğul: `workspaceOf`, `time.*`, `steps(n)`, `library.count(n)`, `insights.rangeShort`, `plan.go`). `AppLangProvider` (`app/layout.tsx`'te mount, SSR default `'en'` → hydration uyumlu, effect localStorage okur), `useT()`→`{lang,setLang,t}`. **localStorage anahtarı `clickthru-lang` landing ile ortak**; landing i18n de default `'en'`'e çevrildi (eski `clickthru-landing-lang` birleşti). Çevrilen: login, `/auth/callback`, onboarding (3 adım), workspaces/home, library, insights, new-demo modal, ⌘K paleti, app-sidebar, app-layout, settings (5 tab + **Görünüm'e Dil seçici** eklendi), export dialog. **Dil geçişi:** sidebar hesap menüsü (Globe), Ayarlar→Görünüm→Dil, landing nav. **Studio editör içi Türkçe kalır** (bilinçli; ayrı karar). Onboarding hedefleri lang-değişimine dayanıklı olsun diye **index ile** tutulur. Build yeşil (13 route), tsc temiz.
